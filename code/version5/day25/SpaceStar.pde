...
PImage newGameBtn; // new game button
PImage exitBtn; // exit button
...
void setup() {
  ...
  /** Back Home Button **/
  backHomeBtn = loadImage("backHomeBtn.png"); // back home buttonback home button is loaded in
  backHomeBtn.resize(180, 45); // resizes the back home button
  backHomeBtnX = width-500; // x coordinate of the back home button initialized
  ...
  invincibility.resize(80, 80); // resizes the invincibility image
...
void helpScreen() {
  ...
  image(helpScreen, 0, 0); // help screen image

  image(backHomeBtn, backHomeBtnX, backHomeBtnY); // back home button
...
void playScreen() {
  ...
  image(fuelImg, fuel.fuelX, fuel.fuelY); // places the fuel boost on the screen
  ...
  image(healthImg, health.healthX, health.healthY); // places the health boost on the screen
  ...
  if (enemy.enemyLifeLeft > 0) {
    ...
    if (chapters == "Start" || chapters == "Carry on The Legacy") {
      screens = "End"; // screen is switched to the end
    } else {
      textSize(17);
...
void gameOver() {
  ...
  backgroundY+=6; // scrolls through the new background

  if (backgroundY >= backgroundPic.height) {
    backgroundY = 0;
  } // resets the new background once first image is fully moved through

  image(gameOverPlayer, 10, height-150); // player spaceship displays bottom left
  image(gameOverPlayer, width-200, height-150); // player spaceship displays bottom right
  image(gameOverPlayer, 10, height-680); // player spaceship displays top left
  image(gameOverPlayer, width-200, height-680); // player spaceship displays top right
