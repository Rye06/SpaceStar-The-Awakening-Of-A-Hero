...
class Boosts {
  ...
  final int doubleDamageSpeed = 3; // double damage boost moving speed
  ...
  final int invincibilitySpeed = 4; // invincibility boost moving speed
  ...
  final int fuelSpeed = 4; // fuel boost moving speed
  ...
  final int healthSpeed = 4; // health boost moving speed
  ...
  void doubleDamageCollision() {
    ...
    if (dist(doubleDamageX, doubleDamageY, player.playerSpaceshipX+110, player.playerSpaceshipY+125) <= 127) {
  ...
  void invincibilityCollision() {
    ...
    if (dist(invincibilityX, invincibilityY, player.playerSpaceshipX+110, player.playerSpaceshipY+125) <= 127) {
  ...
  void fuelCollision() {
    ...
    if (dist(fuelX, fuelY, player.playerSpaceshipX+110, player.playerSpaceshipY+125) <= 127) {
  ...
  void healthCollision() {
    ...
    if (dist(healthX, healthY, player.playerSpaceshipX+110, player.playerSpaceshipY+125) <= 127) {
      healthInitPos(); // shifts the health boost to its initial position
      if (player.playerLifeLeft < 100) {
        player.playerLifeLeft+=5; // increase player life left
      } // increases the life only if it is less than 100
