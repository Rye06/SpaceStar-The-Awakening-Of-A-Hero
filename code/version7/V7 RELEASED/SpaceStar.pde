...
PImage optionsPlayer; // option select image (player)
PImage optionsEnemy; // option select image (enemy)
...
PImage cutscene3; // cutscene 3 image
PImage continueBtn; // continue button

/** Options Y Coordinates**/
int optionsPlayerY;
int optionsEnemyY;
...
int highScoreTime; // high score time
...
void setup() {
  ...
  optionsPlayer = loadImage("options-player.png"); // loads the options image (player)
  optionsEnemy = loadImage("options-enemy.png"); // loads the options image (enemy)
  optionsPlayerY = height-440;
  optionsEnemyY = height-430;
  ...
  highScoreTime = 2147483647; // high score time is set to max integer value
  ...
  cutscene3 = loadImage("cutscene3.png");
...
void home() {
  ...
  /** Home Screen Image **/
  image(home, 0, 0);
  image(optionsPlayer, width-680, optionsPlayerY);
  image(optionsEnemy, width-750, optionsEnemyY);

  /** Instructions Text **/
  textSize(20);
  text("USE Up and Down Arrow To Scroll", width-600, height-100);
  text("and Y to Select!", width-510, height-70);
  text("OR", width-420, height-45);
  text("Click the Button", width-500, height-20);

  /** Respond to the Key Pressed, Depending On the Option Selected **/
  if (key == 'Y' || key == 'y') {
    if (optionsPlayerY <= height-390) {
    ...
    } else if (optionsPlayerY >= height-430) {
      screens = "Help";
    }
  }

  /** Scroll with the Option **/
  if (keyCode == DOWN && optionsPlayerY <= height-390) {
    optionsPlayerY += 20;
    optionsEnemyY += 20;
  } else if (keyCode == UP && optionsPlayerY >= height-430) {
    optionsPlayerY -= 20;
    optionsEnemyY -= 20;
  }
...
void helpScreen() {
  ...
  PImage backHomeBtnHover = loadImage("backHomeBtnHover.png");
  backHomeBtnHover.resize(280, 60);
  /** Back Home Button Hover **/
  if (mouseX>backHomeBtnX && mouseX <backHomeBtnX+backHomeBtnW && mouseY>backHomeBtnY && mouseY <backHomeBtnY+backHomeBtnH) {
    image(backHomeBtnHover, backHomeBtnX, backHomeBtnY);
  }

  /** Back Home Button is Clicked **/
  if (mousePressed) {
    if (mouseX>backHomeBtnX && mouseX <backHomeBtnX+backHomeBtnW && mouseY>backHomeBtnY && mouseY <backHomeBtnY+backHomeBtnH) {
      key = 'b'; // key is set to b
...
void playScreen() {
  ...
  if (millis() - startTimer < 3000 && chapters == "Start") {
    textSize(17);
    text("USE W/S TO MOVE UP AND DOWN, AND A/D TO MOVE LEFT AND RIGHT!", (width/2)-300, height/2);
  }
  ...
  if (player.playerLifeLeft > 0) {
    ...
    noStroke();
    float drawPlayerWidth = (player.playerLifeLeft / maxHealth) * rectWidth;
    rect(width-860, height-50, drawPlayerWidth, 30);
  ...
  if ((millis() - enemyCurShootTime > enemyBullet.enemyShootTime) && (player.playerLifeLeft > 0) && (enemy.enemyLifeLeft > 0)) {
    ...
    enemyCurShootTime = millis(); // updates enemy's current shoot time
  ...
  if (enemy.enemyLifeLeft > 0) {
    if (player.playerLifeLeft > 0) {
      ...
      /* Draws The Health Bar */
      noStroke();
      float drawEnemyWidth = (enemy.enemyLifeLeft / maxHealth) * rectWidth;
      rect(width-205, height-630, drawEnemyWidth, 30);
    ...
    if (chapters == "Start" || chapters == "Carry on The Legacy") {
      ...
      PImage continueBtnHover = loadImage("continueBtnHover.png");
      continueBtnHover.resize(200, 50);
      if (mouseX>continueBtnX && mouseX <continueBtnX+continueBtnW && mouseY>continueBtnY && mouseY <continueBtnY+continueBtnH) {
        image(continueBtnHover, continueBtnX-10, continueBtnY); // continue button on screen
      }
    ...
    if (chapters == "The Final One") {
      ...
      image(cutscene3, player.playerSpaceshipX-130, player.playerSpaceshipY-220); // cutscene 3 image
      enemy.enemySpaceshipY = -500; // moves the enemy spaceship off the screen
...
void resetElements() {
  ...
  mainGameSound.stop(); // stops main game music
  mainGameSound.loop(); // loops main game music
...
void gameOver() {
  ...
  image(gameOverPlayer, 20, height-150); // player spaceship displayed bottom left
  ...
  textAlign(CENTER);
  text(finalName, width-470, height-250);
  textAlign(TOP-LEFT);
  ...
  boolean highTime = false; // sees whether high score time was beat or not
  /** High Score Time Text **/
  if (highScoreTime == 2147483647) {
    text("HIGH SCORE HAS BEEN SET TO: " + totalTimer/1000 + " seconds", (width/2)-260, (height/2)-50);
  } // first time the game finishes
  else {
    text("YOUR LAST HIGH SCORE TIME: " + highScoreTime + " seconds", (width/2)-240, (height/2)-50);
    if (totalTimer/1000 < highScoreTime) {
      highTime = true;
      textSize(25);
      text("NEW HIGH SCORE TIME HAS BEEN SET: " + totalTimer/1000 + " seconds!", (width/2)-270, (height/2)-10);
    } // if the new total time was less than the high score time
  }
  if (key == 'N' || key == 'n') {
    if (highScoreTime == 2147483647 || highTime) {
      highScoreTime = totalTimer/1000; // sets the high score time to the newest one
    }
...
void keyPressed() {
  ...
  if (dist(homePlanetX+750, homePlanetY+735, player.playerSpaceshipX+135, player.playerSpaceshipY+125) <= 236) {
